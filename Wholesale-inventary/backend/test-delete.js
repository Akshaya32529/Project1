...
async function run() {
  try {
    ...
    if (!staffUser) {
      ...
      const createRes = await fetch('http://localhost:5000/api/auth/register', {
        method: 'POST',
        headers: { 
      ...
      // Re-fetch users
      const reFetch = await fetch('http://localhost:5000/api/auth/users', {
        method: 'GET',
        headers: { 'Authorization': `Bearer ${token}` }
      });
...
async function attemptDelete(userId, token) {
  ...
  const deleteRes = await fetch(`http://localhost:5000/api/auth/users/${userId}`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` }
  });
